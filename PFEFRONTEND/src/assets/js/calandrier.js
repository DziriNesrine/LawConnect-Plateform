const currentDayDOM = document.getElementById("cur-day"),
  ...
  popt = document.getElementById("inputMDEx1")
...
function applyTheme() {
	document.querySelector(
		"table"
	).style.boxShadow = `0px 0px 0px -0px ${"#FFF"}`;

	document.querySelector(
		".color"
	).style.backgroundColor = `${"#D2AE6D"}`;
		...
		for (let j = 0; j < 7; j++) {
			...
			if (
				...
				if (currentWeek[j].state !== "currMonth") {
					week.children[j].style.backgroundColor = "#D2AE6D";
...
saveBtnInPopup.addEventListener("click", () => {
    var datetime= popt.value

    if(currentFullMonth.month_idx.toString().length==1){
      currentFullMonth.month_idxx = (currentFullMonth.month_idx+1)
      currentFullMonth.month_idx = "0"+(currentFullMonth.month_idx+1)

    }else{
      currentFullMonth.month_idx = (currentFullMonth.month_idx+1)
      currentFullMonth.month_idxx = (currentFullMonth.month_idx+1)

    }
    if (getSelectedNoteDay.toString().length==1){

      getSelectedNoteDay="0"+getSelectedNoteDay

    }else {
      getSelectedNoteDay=getSelectedNoteDay
	...
	const noteDate =
		currentFullMonth.year +
		"-" +
		(currentFullMonth.month_idx) +
		"-" +
		getSelectedNoteDay +"T"+datetime+":00.000Z";
    console.log("click" , noteDate)
 ...
 console.log("hhhhhhhh",nDate)
 var x= true

	if (true) { //methode post
      fetch("http://localhost:5000/users/date",
      {
          ...
          method: "POST",
          body: JSON.stringify({date : noteDate , avocatID:localStorage.getItem("idAV")})
      })
...
deleteBtnInPopup.addEventListener("click", () => {
	if (getSelectedNoteId == "current-day") {
		noteDate =
			state.todayYear + " " + (state.todayMonth + 1) + " " + state.todayDate;
		notes = notes.filter(note => note.date !== nDate);
		document
