const holidays = [
  ...
  function loadCalendar() {
    ...
    for (let i = 1; i <= dayInMonth + emptyDays; i++) {
      ...
      // Appliquer le style à chaque élément de jour
      ...
      dayBox.style.boxShadow = '0px 0px 3px #cbd4c2';
      dayBox.style.color = '#7f8fa6';
      ...
      // Ajouter le style de survol
      dayBox.addEventListener("mouseover", () => {
        dayBox.style.backgroundColor = 'rgba(112, 111, 211, 0.1)';
        dayBox.style.color = '#706fd3';
      });
      ...
      if (i > emptyDays) {
        ...
        if (i - emptyDays === day && navigation == 0) {
          ...
          // Appliquer le style au jour actuel
          dayBox.style.backgroundColor = '#706fd3';
  ...
  function buttons() {
    ...
    btnSave.addEventListener("click", function () {
      if (txtTime.value) {
        txtTime.classList.remove("error");
        events.push({
          date: clicked,
          title: txtTime.value.trim(),
        });
        txtTime.value = "";
        localStorage.setItem("events", JSON.stringify(events));
        closeModal();
      } else {
        txtTime.classList.add("error");
      }
    });
